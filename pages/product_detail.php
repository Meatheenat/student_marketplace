...
// ระบบนับยอดวิวแบบกันปั๊ม (Session Based)
...
<style>
    ...
    body { background-color: var(--solid-bg) !important; color: var(--solid-text); transition: background 0.3s ease; }

    .product-master-wrapper { max-width: 1100px; margin: 40px auto; padding: 0 20px; opacity: 0; transform: translateY(20px); animation: revealPage 0.8s cubic-bezier(0.16, 1, 0.3, 1) forwards; }
    @keyframes revealPage { to { opacity: 1; transform: translateY(0); } }

    .product-main-card { display: grid; grid-template-columns: 1.1fr 0.9fr; gap: 50px; background: var(--solid-card); border: 2px solid var(--solid-border); border-radius: 32px; padding: 40px; box-shadow: 0 20px 40px rgba(0, 0, 0, 0.05); margin-bottom: 50px; }

    .gallery-wrapper { display: flex; flex-direction: column; gap: 15px; }
    .img-showcase { position: relative; border-radius: 24px; overflow: hidden; background: #000; border: 1px solid var(--solid-border); height: 450px; cursor: zoom-in; }
    .img-showcase img { width: 100%; height: 100%; object-fit: cover; transition: transform 0.6s cubic-bezier(0.23, 1, 0.32, 1), opacity 0.3s ease; }
    .img-showcase:hover img { transform: scale(1.05); }
    .img-animating { opacity: 0.5; transform: scale(0.98); }

    .thumbnail-track { display: flex; gap: 12px; overflow-x: auto; padding-bottom: 5px; }
    ...
    .thumb-btn { width: 80px; height: 80px; flex-shrink: 0; border-radius: 16px; border: 3px solid transparent; overflow: hidden; cursor: pointer; opacity: 0.6; transition: all 0.3s ease; }
    .thumb-btn img { width: 100%; height: 100%; object-fit: cover; }
    .thumb-btn:hover { opacity: 0.9; transform: translateY(-3px); }
    .thumb-btn.active { border-color: var(--solid-primary); opacity: 1; box-shadow: 0 4px 12px rgba(99, 102, 241, 0.3); }

    .product-title-h1 { font-size: 2.8rem; font-weight: 900; letter-spacing: -1.5px; line-height: 1.1; margin-bottom: 15px; color: var(--solid-text); }
    .price-large { font-size: 2.5rem; font-weight: 900; color: var(--solid-primary); margin: 25px 0; display: flex; align-items: center; gap: 10px; }

    .tag-pill { background: var(--solid-bg); border: 1px solid var(--solid-border); color: var(--solid-text); padding: 6px 16px; border-radius: 12px; font-size: 0.8rem; font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px; }

    .btn-buy-now { flex: 1; padding: 20px; border-radius: 18px; background: var(--solid-primary); color: #fff !important; font-weight: 800; font-size: 1.2rem; border: none; cursor: pointer; display: flex; align-items: center; justify-content: center; gap: 12px; box-shadow: 0 10px 25px rgba(79, 70, 229, 0.3); transition: 0.3s cubic-bezier(0.175, 0.885, 0.32, 1.275); }
    .btn-buy-now:hover { transform: translateY(-5px); box-shadow: 0 15px 30px rgba(79, 70, 229, 0.5); }
    .btn-chat-seller { padding: 20px 30px; border-radius: 18px; background: var(--solid-card); border: 2px solid var(--solid-border); color: var(--solid-text); font-weight: 800; font-size: 1.2rem; cursor: pointer; transition: 0.3s; }
    .btn-chat-seller:hover { background: var(--solid-bg); border-color: var(--solid-primary); color: var(--solid-primary); }

    .shop-portal { background: var(--solid-bg); border: 2px solid var(--solid-border); border-radius: 24px; padding: 25px; margin-top: 40px; display: flex; align-items: center; justify-content: space-between; }
    .shop-avatar { width: 60px; height: 60px; background: var(--solid-primary); color: #fff; border-radius: 16px; display: flex; align-items: center; justify-content: center; font-size: 1.5rem; }

    .star-rating-solid { display: flex; flex-direction: row-reverse; justify-content: flex-end; gap: 10px; }
    .star-rating-solid input { display: none; }
    .star-rating-solid label { font-size: 2.2rem; color: #cbd5e1; cursor: pointer; transition: 0.2s; }
    .star-rating-solid input:checked ~ label, .star-rating-solid label:hover, .star-rating-solid label:hover ~ label { color: var(--solid-warning); }

    .review-card-solid { background: var(--solid-card); border: 2px solid var(--solid-border); border-radius: 24px; padding: 30px; margin-bottom: 20px; transition: 0.3s; opacity: 0; transform: translateX(-20px); }
    .review-card-solid.show { opacity: 1; transform: translateX(0); }

    @media (max-width: 992px) { .product-main-card { grid-template-columns: 1fr; padding: 25px; } .img-showcase { height: 400px; } }

    .modal-overlay { position: fixed; z-index: 10000; left: 0; top: 0; width: 100%; height: 100%; background: rgba(15, 23, 42, 0.9); display: none; align-items: center; justify-content: center; }
    .modal-solid { background: var(--solid-card); padding: 40px; border-radius: 32px; width: 90%; max-width: 480px; border: 2px solid var(--solid-border); }
</style>
...
<div class="product-master-wrapper">
        ...
        <div class="product-details-content">
            ...
            <div style="display: flex; gap: 15px; margin-bottom: 40px;">
                <?php if ($user_id && $user_id != $product['owner_id']): ?>
                    ...
                    <a href="chat.php?user=<?= $product['owner_id'] ?>" class="btn-chat-seller" style="display: flex; align-items: center; justify-content: center;">
            ...
            <div class="shop-portal">
                ...
                <div style="display:flex; gap:10px; align-items:center;">
                    <?php if(!empty($product['contact_line'])): ?>
                        <a href="https://line.me/ti/p/~<?= e($product['contact_line']) ?>" target="_blank" style="color:#06c755; font-size:1.6rem;"><i class="fab fa-line"></i></a>
                    <?php endif; ?>
                    <?php if(!empty($product['contact_ig'])): ?>
                        <a href="https://www.instagram.com/<?= e($product['contact_ig']) ?>/" target="_blank" style="color:#e1306c; font-size:1.6rem;"><i class="fab fa-instagram"></i></a>
                    <?php endif; ?>
                </div>
            </div>
    ...
    <div class="reviews-master-box">
        ...
        <?php if (isLoggedIn()): ?>
            <?php $spam_check_ui = canUserReview($user_id, $product_id); if ($spam_check_ui['status']): ?>
            <div class="product-main-card" style="grid-template-columns: 1fr; margin-bottom: 40px;">
                ...
                <form method="POST">
                    ...
                    <button type="submit" name="submit_review" class="btn-buy-now" style="width: auto; padding: 15px 50px; margin-top: 20px;">POST REVIEW</button>
                </form>
            </div>
            <?php else: ?>
                <div style="text-align: center; padding: 40px; background: rgba(99, 102, 241, 0.05); border-radius: 24px; border: 2px solid var(--solid-border); margin-bottom: 40px;">
                    <p style="font-weight: 800; color: var(--solid-primary); margin: 0;"><i class="fas fa-info-circle"></i> <?= $spam_check_ui['message'] ?></p>
                </div>
            <?php endif; ?>
        <?php endif; ?>
                            ...
                            <div style="flex: 1;">
                                <div style="display: flex; justify-content: space-between; align-items: flex-start;">
                                    ...
                                    <div style="display: flex; gap: 10px;">
                                        <button onclick="openReportModal(<?= $rev['id'] ?>, 'comment')" class="toolbar-icon" style="color: var(--solid-border); border:none; background:none; cursor:pointer;"><i class="fas fa-flag"></i></button>
                                        <?php if (isset($_SESSION['role']) && ($_SESSION['role'] === 'admin' || $_SESSION['role'] === 'teacher')): ?>
                                        ...
                                        <?php if ($user_id == $rev['author_id']): ?>
                                            <button type="button" onclick="openEditMyReview(<?= $rev['id'] ?>, <?= $rev['rating'] ?>, '<?= e(str_replace(["\r", "\n"], ' ', $rev['comment'])) ?>')" style="background:none; border:none; color:var(--solid-primary); font-size:0.8rem; cursor:pointer; font-weight:800; padding:0;"><i class="fas fa-edit"></i> Edit</button>
                                            <a href="product_detail.php?id=<?= $product_id ?>&action=delete_my_review&rev_id=<?= $rev['id'] ?>" onclick="return confirm('ลบรีวิวนี้?')" style="text-decoration:none; color:var(--solid-danger); font-size:0.8rem; font-weight:800;"><i class="fas fa-trash"></i> Delete</a>
                                        <?php endif; ?>
                                    </div>
                                </div>
...
<div id="reportModal" class="modal-overlay">
    <div class="modal-solid">
        <div style="text-align: center; margin-bottom: 30px;">
            <div style="width: 80px; height: 80px; background: rgba(239,68,68,0.1); color: var(--solid-danger); border-radius: 50%; display: flex; align-items: center; justify-content: center; margin: 0 auto 15px; font-size: 2.2rem;"><i class="fas fa-shield-alt"></i></div>
        ...
        <form action="../auth/submit_report.php" method="POST">
            ...
            <div style="display:flex; gap:12px; margin-top: 30px;">
                ...
                <button type="submit" class="btn-buy-now" style="flex:1; font-size: 1rem; padding: 15px; background: var(--solid-danger);">SUBMIT</button>
            </div>
        </form>
    </div>
</div>
...
<div id="deleteCommentModal" class="modal-overlay">
    <div class="modal-solid" style="border-color: var(--solid-danger);">
        ...
        <form action="../admin/admin_delete_comment.php" method="POST">
            ...
            <input type="hidden" name="product_id" value="<?= $product_id ?>">
            <div class="form-group">
                <textarea name="reason" class="form-control" required style="width:100%; min-height:120px; border-radius:18px; padding:15px; border: 2px solid var(--solid-border); background: var(--solid-bg);" placeholder="Reason..."></textarea>
            </div>
            <div style="display:flex; gap:12px; margin-top: 30px;">
                <button type="button" onclick="closeDeleteCommentModal()" class="btn-chat-seller" style="flex:1;">CANCEL</button>
                <button type="submit" class="btn-buy-now" style="flex:1; background: var(--solid-danger);">DELETE</button>
            </div>
        </form>
    </div>
</div>
    ...
    <div class="modal-solid" style="border-color: var(--solid-danger);">
        ...
        <form action="../admin/admin_delete_product.php" method="POST">
            ...
            <div style="display:flex; gap:12px; margin-top: 30px;">
                ...
                <button type="submit" class="btn-buy-now" style="flex:1; background: var(--solid-danger);">REMOVE</button>
            </div>
        </form>
    </div>
</div>
...
<script>
    function changeMainImage(imgUrl, clickedThumb) {
        ...
        setTimeout(() => { mainImage.src = imgUrl; mainImage.classList.remove('img-animating'); }, 150);
    }

    const observer = new IntersectionObserver((entries) => {
        entries.forEach((entry, index) => {
            if (entry.isIntersecting) { setTimeout(() => { entry.target.classList.add('show'); }, index * 100); }
        });
    }, { threshold: 0.1 });
    document.querySelectorAll('.stagger-reveal').forEach(el => observer.observe(el));

    function openReportModal(id, type) { document.getElementById('report_target_id').value = id; document.getElementById('report_target_type').value = type; document.getElementById('reportModal').style.display = 'flex'; }
    function closeReportModal() { document.getElementById('reportModal').style.display = 'none'; }
    function openDeleteCommentModal(id, name) { document.getElementById('delete_comment_id').value = id; document.getElementById('target_user_name').innerText = name; document.getElementById('deleteCommentModal').style.display = 'flex'; }
    ...
    function openEditMyReview(id, rating, comment) { document.getElementById('edit_rev_id').value = id; document.getElementById('edit_rev_rating').value = rating; document.getElementById('edit_rev_comment').value = comment; document.getElementById('editMyReviewModal').style.display = 'flex'; }

    document.getElementById('main-wish-btn').addEventListener('click', function() {
        const btn = this; const icon = btn.querySelector('i'); const productId = btn.dataset.id;
        btn.style.transform = "scale(0.8)"; setTimeout(() => btn.style.transform = "scale(1)", 150);
        fetch('../auth/toggle_wishlist.php', {
            method: 'POST', headers: { 'Content-Type': 'application/x-www-form-urlencoded' }, body: 'product_id=' + productId
        }).then(res => res.json()).then(data => {
            if (data.status === 'added') { icon.classList.replace('far', 'fas'); btn.style.color = 'var(--solid-danger)'; }
            else if (data.status === 'removed') { icon.classList.replace('fas', 'far'); btn.style.color = 'var(--solid-border)'; }
        }).catch(err => console.error("Wishlist Error:", err));
    });

    window.onclick = function(event) {
        const reportM = document.getElementById('reportModal'); const commentM = document.getElementById('deleteCommentModal'); const productM = document.getElementById('deleteProductModal');
